...
class CourseClass:
    def __init__(self, root):
        ...
        title = Label(self.root, text="Manage Course Details", font=(
            "Baloo Bhai 2", 20, "bold"), bg="#033054", fg="white").place(x=10, y=15, width=1250, height=35)
        ...
        lbl_duration = Label(self.root, text="Faculty Name", font=(
        ...
        # lbl_facultyname = Label(self.root, text="Faculty Name", font=(
        ...
        # txt_facultyname = Entry(self.root, textvariable=self.var_facultyname, font=(
        #     "goudy old style", 15, 'bold'), bg='lightyellow').place(x=150, y=180, width=200)
        ...
        btn_search = Button(self.root, text='Search', font=(
            "Goudy old style", 15, "bold"), bg="#03a9f4", fg="white", cursor="hand2", command=self.search).place(x=1070, y=60, width=190, height=28)
        ...
        self.CourseTable.heading("duration", text="Faculty Name")
        self.CourseTable.heading("charges", text="Charges")
        # self.CourseTable.heading("facultyname", text="Faculty name")
        ...
        # self.CourseTable.column("facultyname", width=100)
    ...
    def delete(self):
        ...
        try:
            if self.var_course.get() == "":
                ...
                if row == None:
                    messagebox.showerror(
                        "Error", "Please select Course from the list first ", parent=self.root)
                else:
    ...
    def get_data(self, ev):
        ...
        # self.var_facultyname.set(row[4])
        ...
        try:
            if self.var_course.get() == "":
                ...
                if row != None:
                    ...
                    con.commit()
            messagebox.showinfo(
                "Error", "Course Added Successfully", parent=self.root)
            self.show()
    ...
    def update(self):
        ...
        try:
            if self.var_course.get() == "":
                ...
                con.commit()
                messagebox.showinfo(
                    "Error", "Course Update Successfully", parent=self.root)
                self.show()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to {str(ex)}")
